...
#if defined(GD32F30x)
  #define GD32_TIMER1       TIMER4    // for STM32F103 (tone uses timer3, servo uses timer2)
  #define GD32_TIMER2       TIMER2    // don't use the tone() function
  #define GD32_TIMER3       TIMER1    // same use as prior to this point (but different order)
  #define GD32_TIMER4       TIMER3
#endif
#if defined(TASKS_HWTIMER1_ENABLE) || defined(TASKS_HWTIMER2_ENABLE) || defined(TASKS_HWTIMER3_ENABLE) || defined(TASKS_HWTIMER4_ENABLE)
bool HAL_HWTIMER_INIT(uint8_t priority, HardwareTimer * hwtimer, void (*wrapper)()) {
   ...
   // hwtimer->setCaptureCompare(TIMER_CHANNEL, 1); // Interrupt 1 count after each update
    ...
    hwtimer->start();                   // start the timer counting
...
//  #define HAL_HWTIMER1_SET_PERIOD() WRITE_REG(GD32_TIMER1->ARR, _nextPeriod1)
  #define HAL_HWTIMER1_SET_PERIOD() hwtimer1->setPeriodTime(_nextPeriod1, FORMAT_MS)
  ...
  #define HAL_HWTIMER2_SET_PERIOD() hwtimer2->setPeriodTime(_nextPeriod2, FORMAT_MS)
  ...
  #define HAL_HWTIMER3_SET_PERIOD() hwtimer3->setPeriodTime(_nextPeriod3, FORMAT_MS)
  ...
  #define HAL_HWTIMER4_SET_PERIOD() hwtimer4->setPeriodTime(_nextPeriod4, FORMAT_MS)
